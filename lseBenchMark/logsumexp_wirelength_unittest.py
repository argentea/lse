...
class LogSumExpWirelengthOpTest(unittest.TestCase):
    def test_logsumexp_wirelength_random(self):
        ...
        #np.random.seed(123)
        num_pins = np.random.randint(2, 20)
        pin_pos = np.random.rand(num_pins, 2).astype(np.float64) * 10
        net2pin_map = np.array([np.arange(num_pins)])
        print("num_pins = %d" % (num_pins))
        print("pin_pos")
        print(pin_pos)
        print("net2pin_map")
        print(net2pin_map)
        ...
        net_weights = np.random.rand(len(net2pin_map)).astype(np.float64)
        print("net_weights")
        print(net_weights)
        ...
        with open("net10.txt", "w") as f:
            content = "%d pins\n" % (num_pins)
            content += "net_weight : %6g\n" % (net_weights[0])
            content += "gamma : %6g\n" % (gamma.item())
            content += "# pin positions\n"
            for i in range(num_pins):
                content += "pin%d : %6E %6E\n" % (i, pin_pos[i, 0], pin_pos[i, 1])
            content += "# solution\n"
            content += "LSE : %6E\n" % (golden.data)
            content += "# pin gradients\n"
            for i in range(num_pins):
                content += "grad pin%d : %6E %6E\n" % (i, golden_grad.view(2, -1)[0, i], golden_grad.view(2, -1)[1, i])
            f.write(content)
